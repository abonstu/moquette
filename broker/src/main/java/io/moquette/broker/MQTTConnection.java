...
final class MQTTConnection
{
	...
	void processConnect( MqttConnectMessage msg )
	{
		...
		if( clientId == null || clientId.length() == 0 )
		{
			...
			{
				LOG.warn(
						"MQTT client ID cannot be empty for persistent session. Username: {}, channel: {}",
						username,
						channel );
				abortConnection( CONNECTION_REFUSED_IDENTIFIER_REJECTED );
				return;
			...
			LOG.debug(
					"Client has connected with integration generated id: {}, username: {}, channel: {}",
					clientId,
					username,
					channel );
		}
	...
	private void initializeKeepAliveTimeout( Channel channel, MqttConnectMessage msg, String clientId )
	{
		...
		LOG.debug(
				"Connection has been configured CId={}, keepAlive={}, removeTemporaryQoS2={}, idleTime={}",
				clientId,
				keepAlive,
	...
	private boolean login( MqttConnectMessage msg, final String clientId )
	{
		...
		if( msg.variableHeader().hasUserName() )
		{
			...
			final InetSocketAddress socketAddress = (InetSocketAddress) this.channel.remoteAddress();
			final String clientAddress = NettyUtils.clientAddress( this.channel ) != null ? NettyUtils.clientAddress( this.channel ) : socketAddress.getHostString();
			if( ! authenticator.checkValid( clientId, login, pwd, clientAddress ) )
			{
	...
	void processPublish( MqttPublishMessage msg )
	{
		...
		LOG.trace(
				"Processing PUBLISH message. CId={}, topic: {}, messageId: {}, qos: {}",
				clientId,
				topicName,
		...
		{
			case AT_MOST_ONCE:
				postOffice.receivedPublishQos0( this, topic, username, clientId, payload, retain, msg );
				break;
			case AT_LEAST_ONCE:
			{
	...
	void sendPublish( MqttPublishMessage publishMsg )
	{
		...
		{
			LOG.trace(
					"Sending PUBLISH({}) message. MessageId={}, CId={}, topic={}, payload={}",
					qos,
					packetId,
					clientId,
					topicName,
		...
		{
			LOG.debug(
					"Sending PUBLISH({}) message. MessageId={}, CId={}, topic={}",
					qos,
					packetId,
					clientId,
					topicName );
